  ...
  return (
    <Card>
      <CardContent className="p-4">
        <h3 className="font-semibold">Offer Stage</h3>
        {application.offerLetter ? (
          <p className="mt-2 text-sm text-emerald-400">Offer sent</p>
        ) : (
          <>
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-xl border border-white/20 bg-[#1a1f26] p-6 shadow-xl">
            <Dialog.Title className="text-lg font-semibold">
              Offer Letter
            </Dialog.Title>
            <textarea
              className="mt-4 h-64 w-full resize-none rounded-lg border border-white/20 bg-white/5 p-4 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#3b82f6]"
              value={offerText}
